...
with open(csv_filename, 'a', newline='') as csvfile:
    ...
    for filename in os.listdir(image_folder):
        if filename.endswith(".jpg") or filename.endswith(".png"):
            ...
            corners, ids, rvecs, tvecs = tag_detection_provider.detect_marker(undistord_image, 'aruco', 0.6, 'DICT_4X4_100')
            print("==============================")
            if ids is not None:
                ...
                for i in range(len(ids)):
                    if tag_id is None or ids[i] == tag_id:
                        ...
                        #calculer norme du vecteur de translation
                        print("image name",filename)
                        print(f"x:{tvec[0][0]} y:{tvec[0][1]} z:{tvec[0][2]}")
                        print("norme:",np.linalg.norm(tvec))
                        """
                        ...
                        axis_length = 0.8  # longueur des axes du trièdre
                        ...
                        """
                        cv2.drawFrameAxes(undistord_image, calib_data[0], calib_data[1], rvec, tvec, 0.8)
